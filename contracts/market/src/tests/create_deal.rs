...
use crate::msg::{AllDealsResponse, DealsByCreatorResponse, ExecuteMsg, InstantiateMsg, QueryMsg};
...
fn create_deal_works() {
    ...
    let resp: DealsByCreatorResponse = app
        .wrap()
        .query_wasm_smart(market_addr.clone(), &QueryMsg::DealsByCreator { creator: not_a_scammer.to_string() })
        .unwrap();
    ...
    assert_eq!(resp.deals.len(), 0, "expected zero deal from first creator because expired");
